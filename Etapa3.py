...
def create_table_v():
    print("Creando tabla ventas...") # Para probar que se ejecuta la función
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
...
class Carrito:
    ...
    def comprar(self):
        for item in self.items:
            sql = f'INSERT INTO ventas VALUES ({item.codigo}, "{item.descripcion}", {item.cantidad}, {item.precio});'
            self.cursor.execute(sql)
            self.conexion.commit()
        self.items.clear()
        return jsonify({'message': 'Venta concretada.'}), 200



# -------------------------------------------------------------------
# Definimos la clase "Ventas"
# -------------------------------------------------------------------
class Ventas:
    def __init__(self):
        self.conexion = get_db_connection()
    ...
    def agregar_producto(self, codigo, descripcion, cantidad, precio):
        self.cursor.execute("INSERT INTO ventas VALUES (?, ?, ?, ?)", (codigo, descripcion, cantidad, precio))
        self.conexion.commit()
        return jsonify({'message': 'Producto agregado correctamente en Ventas.'}), 200
    ...
    def listar_productos(self):
        ...
        for row in rows:
            codigo, descripcion, cantidad, precio = row
            ventas = {'codigo': codigo, 'descripcion': descripcion, 'cantidad': cantidad, 'precio': precio}
            ventass.append(ventas)
        return jsonify(ventass), 200

    def eliminar_producto(self, codigo):
        self.cursor.execute("DELETE FROM ventas WHERE codigo = ?", (codigo,))
        if self.cursor.rowcount > 0:
            self.conexion.commit()
            return jsonify({'message': 'Producto eliminado correctamente.'}), 200
        return jsonify({'message': 'Producto no encontrado.'}), 404
...
ventas = Ventas()           # Instanciamos un inventario
...
# A - Ruta para obtener el contenido del carrito
@app.route('/carrito/comprar', methods=['GET'])
def comprar_carrito():
    return carrito.comprar()

# B - Ruta para obtener el contenido de ventas
@app.route('/ventas', methods=['GET'])
def obtener_ventas():
    return ventas.listar_productos()
